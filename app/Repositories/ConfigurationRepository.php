...
class ConfigurationRepository
{
    ...
    {
        $config_type = $params['config_type'] ?? null;
        ...
        foreach ($params as $key => $value) {
            if (!in_array($key, ['config_type','providers']) && (!in_array($key, config('system.private_config_variables'), true) || (in_array($key, config('system.private_config_variables'), true) && $value !== config('system.hidden_field')))) {
    ...
    {
        $config_type = $params['config_type'] ?? null;
        $social_login = $params['social_login'] ?? 0;

        if ($config_type !== 'authentication' || ! $social_login) {
            return;
        }

        $providers = $params['providers'] ?? 0;
    ...
    {
        $config_type = $params['config_type'] ?? null;
        $locale = $params['locale'] ?? config('app.locale');
    ...
    public function setDefault()
    {
        ...
        $default_config = $system_variables['default_config'] ?? [];
        ...
        date_default_timezone_set(config('config.timezone') ? : 'Europe/Moscow');
        \App::setLocale(config('config.locale') ? : 'ru');
    ...
    public function setEnv($type = null)
    {
        ...
        if ($type === 'system') {
            envu(['APP_DEBUG' => !\App::environment('production') && config('config.error_display')]);
        }
